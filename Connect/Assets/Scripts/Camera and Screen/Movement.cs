...
public class Movement : MonoBehaviour
{
    ...
    public void ToggleMove(bool newToggle)
    {
        onOverlay = newToggle;
    }
    ...
    void Update()
    {
        ...
        if (onOverlay && ((67.5f < cam.orthographicSize && Input.mouseScrollDelta.y > 0) || (1080 > cam.orthographicSize && Input.mouseScrollDelta.y < 0)))
        {
            ...
            zoomCoords[0] = Mathf.Clamp(transform.position.x + ((zoomFactor - 1) * newMouseCoords[0]) / zoomFactor, -817.5f, 817.5f);
            zoomCoords[1] = Mathf.Clamp(transform.position.y + ((zoomFactor - 1) * newMouseCoords[1]) / zoomFactor, -817.5f, 817.5f);
